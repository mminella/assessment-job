 ...
 */
@Component public class ClosedAsInvalidPerMonthTasklet extends ReportTasklet {

    private final RestOperations restTemplate;

    private SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd");

    public ClosedAsInvalidPerMonthTasklet(RestOperations restTemplate, Map<ReportKey, List<Long>> report,
    ...
    @Override public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) throws Exception {
        ...
        String declinedQuery =
            this.properties.getProjectRepo() + " is:issue closed:%s is:closed label:\"status: declined\"";
        String invalidQuery =
            this.properties.getProjectRepo() + " is:issue closed:%s is:closed label:\"status: invalid\"";
        long issueCount = 0;
